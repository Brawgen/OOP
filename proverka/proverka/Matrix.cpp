...
void Matrix::Show()			//Показать матрицу
{
	for (int i = 0; i < this->line; i++)
	{
		for (int j = 0; j < this->column; j++)
...
Matrix Matrix::Multiplication(double factor) //Умножение матрицы на число
{
	...
	for (int i = 0; i < this->line; i++)
	{
		for (int j = 0; j < this->column; j++)
		{
			Multi_vect[i][j] = (factor * (this->matr[i][j]));
		}
	}
...
Matrix Matrix::Transpose()		//Транспонирование
{
	...
	{
		std::cout << "Невозможно транспонировать нулевую матрицу!" << "\n";
		Matrix empty;
		return empty;
	}
...
double Matrix::Deter()					//Проверка на квадратную матрицу и вызов функции для нахождения опрделителя вектора
{
	...
	{
		std::cout << "Невозможно найти определитель!" << "\n";
		std::cout << "Матрица не квадратная или нулевая" << "\n";
		return NAN;
	}
	double Det;
...
Matrix Matrix::Reverse_Mat()
{
	if ((this->column != this->line) or (this->line == 0))
	{
		std::cout << "Невозможно найти обратную матрицу!!!" << "\n";
		std::cout << "Матрица не квадратная или нулевая" << "\n";
		Matrix empty;
		return empty;
	}
